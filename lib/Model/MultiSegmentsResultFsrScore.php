...
class MultiSegmentsResultFsrScore implements ModelInterface, ArrayAccess
{
    ...
    public function setModelCode($model_code)
    {
        if (!is_null($model_code) && (mb_strlen($model_code) > 6)) {
            throw new \InvalidArgumentException('invalid length for $model_code when calling MultiSegmentsResultFsrScore., must be smaller than or equal to 6.');
        }
    ...
    public function setModelTitle($model_title)
    {
        if (!is_null($model_title) && (mb_strlen($model_title) > 30)) {
            throw new \InvalidArgumentException('invalid length for $model_title when calling MultiSegmentsResultFsrScore., must be smaller than or equal to 30.');
        }
